...
This is a **fork** of the archived `splunk/k8s-metrics`. Upstream is read-only and the image it used to
publish, `splunk/k8s-metrics:1.2.3`, **has been deleted from Docker Hub** — a cluster that loses its cached
...
```sh
gh secret list --repo "ephico2real2/k8s-metrics"
```
...
```
docker.io/ephico2real/k8s-metrics:1.2.3-h1-g<sha>     immutable — pin THIS in a cluster
docker.io/ephico2real/k8s-metrics:1.2.3-h1            rolling within the hardening pass
quay.io/ephico2real/k8s-metrics:1.2.3-h1-g<sha>       mirror, same digest
...
```sh
gh workflow run publish-image.yaml --repo ephico2real2/k8s-metrics \
...
```
ephico2real/k8s-metrics:1.2.3-h1-g<sha>   # immutable — pin THIS in a cluster
ephico2real/k8s-metrics:1.2.3-h1          # rolling within the hardening pass
```
